...
DEFAULT_CONFIG = Path("pufferlib/config/ocean/drive_bc_paired_offline_fits_car.ini")
DEFAULT_CHECKPOINT = Path(
    "/home/casko/phd-code/PufferDrive/experiments_bc/"
    "bc-car-paired-offline-fits-gpu-20260428-164632/best.pt"
)
...
DEFAULT_OUTPUT_DIR = Path(
    ...
    "bc_car_paired_offline_fits_20260428_164632/trajectory_grids"
)
...
def _action_from_logits(logits, deterministic: bool):
    ...
    if isinstance(logits, (tuple, list)):
        if len(logits) != 1:
            raise ValueError("BC rollout grid supports only single-head discrete policies")
        logits = logits[0]
    if deterministic:
...
def _rollout_one_map(
    *,
    ...
    temp_root = Path(tempfile.mkdtemp(prefix=f"{map_record['source_map_name']}_bc_car_", dir=str(workspace_dir)))
    vecenv = None
    try:
...
def collect_bc_rollouts(
    *,
    ...
    workspace_dir = output_path.parent / ".bc_car_workspace"
    ...
    try:
        for idx, record in enumerate(map_records):
            try:
                print(f"[BC rollout] {idx + 1}/{len(map_records)} {record['display_map_name']}", flush=True)
                rollouts.append(
                    _rollout_one_map(
                        base_args=base_args,
                ...
                print(f"[BC rollout] ERROR {record['display_map_name']}: {type(exc).__name__}: {exc}", flush=True)
    finally:
    ...
    payload = {
        ...
        "reference_model": reference_payload.get("model_name"),
        "rollouts": rollouts,
        "errors": errors,
    }
...
def _action_summary(rollout: dict[str, Any] | None) -> str:
    ...
    return f"steps={int(rollout.get('steps', actions.size))}, {rollout.get('stop_reason', '')}, top={top}"
...
def plot_grid(
    *,
    ...
    bc_by_map = _rollout_by_map(bc_payload)
    ref_by_map = _rollout_by_map(reference_payload)
    gt_by_map = _rollout_by_map(gt_payload)
    map_names = sorted(set(ref_by_map) | set(gt_by_map) | set(bc_by_map))
    ...
    specs = [
        ...
        ("PPO car", ref_by_map, "tab:orange", "--"),
        ("BC car", bc_by_map, "tab:green", "--"),
    ]
    for idx, map_name in enumerate(map_names):
        ...
        ax.set_title(f"{map_name}\nBC {_action_summary(bc_by_map.get(map_name))}", fontsize=9)
        ax.set_aspect("equal", adjustable="box")
        ax.grid(True, alpha=0.2)
        if idx == 0:
            ax.legend(fontsize=8, loc="best")

    bc_rollouts = list(bc_payload.get("rollouts", []))
    deterministic = bool(bc_rollouts[0].get("deterministic", True)) if bc_rollouts else True
    action_mode_note = "argmax actions" if deterministic else "sampled actions"
    note = (
        "Solid blue is the fitted/ground-truth car trajectory. Dashed orange is the existing PPO car rollout on the same maps. "
        "Dashed green is the newly trained BC car checkpoint. Thick segments show the first 32 steps; thin lines show the full collected rollout. "
        f"Circle marks rollout start, x marks the highlighted prefix end. BC rollout uses {action_mode_note} from the 91-way discrete policy."
    )
    fig.suptitle("BC Car Rollouts Compared With GT and PPO Car", fontsize=15)
    ...
    stop_reasons = {}
    for rollout in bc_payload.get("rollouts", []):
        reason = str(rollout.get("stop_reason", ""))
        stop_reasons[reason] = stop_reasons.get(reason, 0) + 1
    return {
        "grid": str(output_path),
        "bc_rollout_count": len(bc_payload.get("rollouts", [])),
        "bc_error_count": len(bc_payload.get("errors", [])),
        "bc_stop_reasons": stop_reasons,
    }


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Collect and plot BC car rollouts against existing GT/PPO car trajectories.")
...
def main() -> None:
    ...
    rollout_path = args.output_dir / "bc-car-paired-offline-fits_rollouts.pt"
    bc_payload = collect_bc_rollouts(
    ...
    outputs = plot_grid(
        ...
        output_path=args.output_dir / "bc_car_gt_ppo_overlay_trajectory_grid.png",
    )
    outputs["bc_rollouts"] = str(rollout_path)
    outputs["errors"] = bc_payload.get("errors", [])
    summary_path = args.output_dir / "bc_car_rollout_grid_summary.json"

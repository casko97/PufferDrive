...
void WriteFrameFromTexture(VideoRecorder *recorder, RenderTexture2D *target, int width, int height) {
    Image img = LoadImageFromTexture(target->texture);
    if (img.format != PIXELFORMAT_UNCOMPRESSED_R8G8B8A8) {
        ImageFormat(&img, PIXELFORMAT_UNCOMPRESSED_R8G8B8A8);
    }
    ImageFlipVertical(&img);
    write(recorder->pipefd[1], img.data, width * height * 4);
    UnloadImage(img);
}
...
void renderTopDownView(Drive *env, Client *client, RenderTexture2D *target, int map_height, int obs, int lasers,
                       int trajectories, int frame_count, float *path, int show_human_logs, int show_grid, int img_width,
                       int img_height, int zoom_in) {
    BeginDrawing();
...
void renderAgentView(Drive *env, Client *client, RenderTexture2D *target, int map_height, int obs_only, int lasers,
                     int show_grid) {
...
int eval_gif(const char *map_name, const char *policy_name, int show_grid, int obs_only, int lasers,
    ...
    RenderTexture2D topdown_target = {0};
    RenderTexture2D agent_target = {0};

    if (render_topdown) {
        topdown_target = LoadRenderTexture(img_width, img_height);
        if (!OpenVideo(&topdown_recorder, filename_topdown, img_width, img_height)) {
            UnloadRenderTexture(topdown_target);
            CloseWindow();
            return -1;
        }
    }
    ...
    if (render_topdown) {
        ...
        for (int i = 0; i < frame_count; i++) {
            if (i % frame_skip == 0) {
                renderTopDownView(&env, client, &topdown_target, map_height, 0, 0, 0, frame_count, NULL,
                                  show_human_logs, show_grid, img_width, img_height, zoom_in);
                WriteFrameFromTexture(&topdown_recorder, &topdown_target, img_width, img_height);
                rendered_frames++;
            }
    ...
    if (render_agent) {
        ...
        for (int i = 0; i < frame_count; i++) {
            ...
            if (i % frame_skip == 0) {
                renderAgentView(&env, client, &agent_target, map_height, obs_only, lasers, show_grid);
                WriteFrameFromTexture(&agent_recorder, &agent_target, img_width, img_height);
                rendered_frames++;
            }
            forward(net, env.observations, (int *)env.actions);
            c_step(&env);

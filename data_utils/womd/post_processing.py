    ...
    try:
        with open(file_path, "r") as file:
            ...
            if not isinstance(data["objects"], list) or not all(
                isinstance(obj, dict) and "position" in obj and "type" in obj for obj in data["objects"]
            ):
                return False

            # Check if "roads" is a list of dictionaries with required "geometry"
            if not isinstance(data["roads"], list) or not all(
                isinstance(road, dict) and "geometry" in road for road in data["roads"]
            ):
                return False

            # Check that each "geometry" in "roads" has valid "x" and "y" coordinates
            for road in data["roads"]:
                if not all(isinstance(geo, dict) and "x" in geo and "y" in geo for geo in road.get("geometry", [])):
                    return False
...
def process_directory(dataset_dir, num_workers=None):
    ...
    group_dirs = [d for d in dataset_path.iterdir() if d.is_dir() and d.name.startswith("group_")]
    ...
    main_dir_files = [f for f in dataset_path.glob("*.json") if not any(g.name in str(f) for g in group_dirs)]
...
def process_all_directories(num_workers=None):
    ...
    directories = ["data/processed/training", "data/processed/testing", "data/processed/validation"]

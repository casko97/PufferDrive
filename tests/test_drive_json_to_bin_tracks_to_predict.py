...
def test_drive_json_to_bin_accepts_integer_tracks_to_predict(tmp_path):
    scenario = {
        "metadata": {
            "sdc_track_index": 0,
            "tracks_to_predict": [0, 2],
    ...
    output_file = tmp_path / "map_tracks_to_predict_ints.bin"
    save_map_binary(scenario, str(output_file), unique_map_id=42)

    with open(output_file, "rb") as f:
        assert _read_int(f) == 0
        assert _read_int(f) == 2
        assert _read_int(f) == 0
        assert _read_int(f) == 2
